...
print(f"[{time.ctime()}] Changing settings in 192.168.10.240")
...
print(f"[{time.ctime()}] Connecting to 192.168.10.240")
...
if switcher.waitForConnection(infinite=False):
    ...
    print(f"[{time.ctime()}] Settings updated on {switcher.atemModel} at 192.168.10.240")
    count += 1
else:
    print(f"[{time.ctime()}] ERROR: no response from 192.168.10.240")
switcher.disconnect()